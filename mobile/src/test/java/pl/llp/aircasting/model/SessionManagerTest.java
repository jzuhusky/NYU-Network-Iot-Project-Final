...
public class SessionManagerTest {
	...
	public void isCalibrating() {
		...
		doReturn(new Session()).when(sessionManager.sessionRepository).loadFully(anyLong(),
				Matchers.<ProgressListener>anyObject());
	...
	public void shouldSaveAdditionalData() {
		...
		verify(sessionManager.sessionRepository).save(Mockito.argThat(new BaseMatcher<Session>() {
			...
			public boolean matches(Object o) {
				...
				return session.getCalibration() == 123 && session.getOffset60DB() == 432
						&& "1.1.1".equals(session.getOSVersion()) && "very old".equals(session.getPhoneModel());
			}
