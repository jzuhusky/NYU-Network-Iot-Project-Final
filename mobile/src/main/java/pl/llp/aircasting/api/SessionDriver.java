...
public class SessionDriver {
	...
	public HttpResult<CreateSessionResponse> create(Session session) {
		...
		PerformRequest builder = http().post().to(SESSIONS_PATH).with(SESSION_KEY, zipped).with(COMPRESSION, "true");
	...
	public HttpResult<Session> show(long id) {
		return http().get().from(USER_SESSION_PATH + id + ".json").into(Session.class);
	}
	...
	public HttpResult<DeleteSessionResponse> deleteSession(Session session) {
		...
		try {
			...
			PerformRequest builder = http().post().to(DELETE_SESSION_PATH).with(SESSION_KEY, zipped).with(COMPRESSION,
					"true");
	...
	public HttpResult<DeleteSessionResponse> deleteStreams(Session session) {
		...
		try {
			String zipped = new String(gzip(copy));
			PerformRequest builder = http().post().to(DELETE_SESSION_STREAMS_PATH).with(SESSION_KEY, zipped)
