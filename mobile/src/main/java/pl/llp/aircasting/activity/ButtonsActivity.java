...
import pl.llp.aircasting.IOTExtension.ExtendedSensorService;
import pl.llp.aircasting.IOTExtension.IOTServerCommandEvent;
import pl.llp.aircasting.IOTExtension.IOTToggleAirCastingEvent;
import pl.llp.aircasting.IOTExtension.UserStopSensingEvent;
...
/**
 * A common superclass for activities that want to display left/right navigation
 * arrows
 */
public abstract class ButtonsActivity extends RoboMapActivityWithProgress implements View.OnClickListener {
	...
	@Inject ExtendedSensorService ess;

	private boolean suppressTap = false;
	private boolean initialized = false;
	private boolean isIOTCasting = false;
	...
	/**
	 * The next tap will not change the state of the toggleable buttons. Use
	 * this if clicking something makes the toggleable buttons toggle.
	 */
	...
	public void onClick(View view) {
		...
		case R.id.toggle_aircasting:
			// If the user decides to kill prog, allow it
			userToggle = true;
			toggleAirCasting();
			break;
		case R.id.edit:
	...
	private void stopAirCasting() {
		if (this.isIOTCasting){
			if (userToggle){
				eventBus.post(new UserStopSensingEvent());
				userToggle = false;
			}
			this.IOTStopAirCasting();
		}else{
			locationHelper.stop();	
	...
	/**
	 * EXTENSION CLASS TO TOGGLE AIRCASTING
	 * FROM OUR EXTENSION SERVICE
	 * 
	 */

	@Subscribe
	public void onEvent(IOTToggleAirCastingEvent event) {
		this.isIOTCasting = true;
		toggleAirCasting();
	}
	/**
	 * Stop Air Casting, usually User UI Input required
	 * This bypasses that....
	 */
	private void IOTStopAirCasting(){
		this.isIOTCasting = false;
		locationHelper.stop();	
		Session session = sessionManager.getSession();
		Long sessionId = session.getId();
		if (session.isEmpty()) {
			...
			sessionManager.stopSession();
			// Default to always save session!!!
			// Manually fill in  descriptions...
			sessionManager.setTitleTagsDescription(
					sessionId,
					"IOT-Session",
					"iot",
					"iot session toggled by remote server");
