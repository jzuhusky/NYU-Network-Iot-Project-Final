...
public class MeasurementStream implements Serializable {
	...
	public MeasurementStream(Sensor sensor) {
		this(sensor.getPackageName(), sensor.getSensorName(), sensor.getMeasurementType(), sensor.getShortType(),
				sensor.getUnit(), sensor.getSymbol(), sensor.getThreshold(VERY_LOW), sensor.getThreshold(MID),
				sensor.getThreshold(LOW), sensor.getThreshold(HIGH), sensor.getThreshold(VERY_HIGH), "none");
	}

	public MeasurementStream(String packageName, String sensorName, String type, String shortType, String unit,
			String symbol, int thresholdVeryLow, int thresholdLow, int thresholdMedium, int thresholdHigh,
			int thresholdVeryHigh) {
		this(packageName, sensorName, type, shortType, unit, symbol, thresholdVeryLow, thresholdLow, thresholdMedium,
				thresholdHigh, thresholdVeryHigh, "none");
	}

	public MeasurementStream(String packageName, String sensorName, String type, String shortType, String unit,
			String symbol, int thresholdVeryLow, int thresholdLow, int thresholdMedium, int thresholdHigh,
			int thresholdVeryHigh, String address) {
	...
	public boolean equals(Object o) {
		...
		if (measurementType != null ? !measurementType.equals(stream.measurementType) : stream.measurementType != null)
			return false;
		if (sensorName != null ? !sensorName.equals(stream.sensorName) : stream.sensorName != null)
			return false;
		if (symbol != null ? !symbol.equals(stream.symbol) : stream.symbol != null)
			return false;
		if (unit != null ? !unit.equals(stream.unit) : stream.unit != null)
			return false;
	...
	public String toString() {
		return "MeasurementStream{" + "measurements=" + measurements + ", sensorName='" + sensorName + '\''
				+ ", measurementType='" + measurementType + '\'' + ", unit='" + unit + '\'' + ", symbol='" + symbol
				+ '\'' + ", sum=" + sum + ", peak=" + peak + '}';
	}
	...
	public enum Visibility {
		VISIBLE, INVISIBLE_DELETED, INVISIBLE_DISCONNECTED, VISIBLE_RECONNECTED;
	}
}
