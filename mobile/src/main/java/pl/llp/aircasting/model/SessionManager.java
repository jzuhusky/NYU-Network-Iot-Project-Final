...
public class SessionManager {
	...
	public synchronized void onEvent(SensorEvent event) {
		...
		if (location != null && sensor != null && sensor.isEnabled()) {
			...
			Measurement measurement = new Measurement(latitude, longitude, value, event.getMeasuredValue(),
					event.getDate());
			if (state.recording().isRecording()) {
	...
	public void updateNote(final Note currentNote) {
		dbQueue.add(new WritableDatabaseTask<Void>() {
			...
			public Void execute(SQLiteDatabase writableDatabase) {
				...
				@Language("SQLite")
				String whereClause = " WHERE " + DBConstants.NOTE_NUMBER + " = " + currentNote.getNumber() + " AND "
						+ DBConstants.NOTE_SESSION_ID + " = " + session.getId();
				writableDatabase.update(DBConstants.NOTE_TABLE_NAME, values, whereClause, null);
				return null;
			}
		});
	}
	//IOT Extension method
	public void setAudioSamplingFrequency(int rate){
		if (audioReader != null)
			audioReader.setSamplingFrequency(rate);
	}
}
