...
/**
 * A class which reads built in input from the mic in a background thread and
...
//        audioManager = (AudioManager) app.getSystemService(Context.AUDIO_SERVICE);
     ...
     * Start this reader.
     *
     * @param rate     The built in sampling rate, in samples / sec.
     * @param block    Number of samples of input to read at a time.
     *                 This is different from the system built in
     *                 buffer size.
    ...
    private void readerRun() {
        ...
        try {
            while (timeout > 0 && audioInput.getState() != AudioRecord.STATE_INITIALIZED) {
                Thread.sleep(50); 
                timeout -= 50;    // This determines the speed of sensing... ????
            }
        } catch (InterruptedException e) {
            ...
            while (running) {
                ...
                synchronized (buffer) {
                    ...
                    if (done) {
                        ...
                        // the built in analyzer.  We don't want to be forced to
                        ...
                        try {
                            ...
                            buffer.wait(MS_BETWEEN_SAMPLES);
                        } catch (InterruptedException e) {
                        }
                    }
                }
    ...
    public void setSamplingRate(int rate){
    	MS_BETWEEN_SAMPLES = 1000 / rate; // (integer division)
    }
    ...
    //MS between samples for IOT extension
    private int MS_BETWEEN_SAMPLES = 1000; // default
