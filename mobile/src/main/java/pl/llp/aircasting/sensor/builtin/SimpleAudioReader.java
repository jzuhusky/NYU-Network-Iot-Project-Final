...
public class SimpleAudioReader extends AudioReader.Listener {
	...
	public void onReadComplete(short[] buffer) {
		...
		if (power != null) {
			...
			SensorEvent event = new SensorEvent(SENSOR_PACKAGE_NAME, SENSOR_NAME, MEASUREMENT_TYPE, SHORT_TYPE, UNIT,
					SYMBOL, VERY_LOW, LOW, MID, HIGH, VERY_HIGH, calibrated);
			...
			if (Constants.isDevMode()) {
				SensorEvent event2 = new SensorEvent("2" + SENSOR_PACKAGE_NAME, "2" + SENSOR_NAME, MEASUREMENT_TYPE,
						SHORT_TYPE, UNIT, SYMBOL, VERY_LOW, LOW, MID, HIGH, VERY_HIGH, 25);
	...
	public void setSamplingFrequency(int rate){
		audioReader.setSamplingRate(rate);
	}
}
