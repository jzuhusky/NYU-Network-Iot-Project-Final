...
public class SensorRepository implements DBConstants {
	public static final String[] SENSOR_FIELDS = new String[] { STREAM_SENSOR_NAME, STREAM_SENSOR_PACKAGE_NAME,
			STREAM_MEASUREMENT_TYPE, STREAM_SHORT_TYPE, STREAM_MEASUREMENT_UNIT, STREAM_MEASUREMENT_SYMBOL,
			STREAM_THRESHOLD_VERY_LOW, STREAM_THRESHOLD_LOW, STREAM_THRESHOLD_MEDIUM, STREAM_THRESHOLD_HIGH,
	...
	private ReadOnlyDatabaseTask<List<Sensor>> readSensorsTask() {
		return new ReadOnlyDatabaseTask<List<Sensor>>() {
			...
			public List<Sensor> execute(SQLiteDatabase readOnlyDatabase) {
				...
				Cursor cursor = readOnlyDatabase.query(distinct, STREAM_TABLE_NAME, SENSOR_FIELDS, null, null, null,
						null, null, null);
				...
				if (cursor.moveToFirst()) {
					while (!cursor.isAfterLast()) {
						...
						Sensor sensor = new Sensor(packageName, name, measurementType, shortType, unit, symbol, veryLow,
