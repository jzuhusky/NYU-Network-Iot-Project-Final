...
public class StreamRepository {
	...
	List<MeasurementStream> findAllForSession(@NotNull final Long sessionId) {
		return airCastingDB.executeReadOnlyTask(new ReadOnlyDatabaseTask<List<MeasurementStream>>() {
			...
			public List<MeasurementStream> execute(SQLiteDatabase readOnlyDatabase) {
				...
				Cursor c = readOnlyDatabase.rawQuery(
						"SELECT * FROM " + STREAM_TABLE_NAME + " WHERE " + STREAM_SESSION_ID + " = " + sessionId + "",
						null);
				...
				while (!c.isAfterLast()) {
					...
					stream = new MeasurementStream(packageName, sensor, type, shortType, unit, symbol, thresholdVeryLow,
							thresholdLow, thresholdMedium, thresholdHigh, thresholdVeryHigh);
	...
	public void saveAll(Collection<MeasurementStream> streamsToSave, long sessionId, SQLiteDatabase writableDatabase) {
		for (MeasurementStream oneToSave : streamsToSave) {
			...
			Log.d(Constants.PERFORMANCE_TAG,
					"Saving " + measurementsToSave.size() + " measurements took: " + s.elapsed(TimeUnit.MILLISECONDS));
		}
	}
	...
	void markForRemoval(MeasurementStream stream, long sessionId, SQLiteDatabase writableDatabase) {
		try {
			...
			Logger.e("Unable to mark stream [" + stream.getId() + "] from session [" + sessionId + "] to be deleted",
					e);
		}
	}
	...
	void deleteSubmitted(SQLiteDatabase writableDatabase) {
		try {
			Cursor cursor = writableDatabase.query(STREAM_TABLE_NAME, null, STREAM_IS_SUBMITTED_FOR_DELETE, null, null,
					null, null);
			cursor.moveToFirst();
	...
	public void markRemovedForRemovalAsSubmitted() {
		airCastingDB.executeWritableTask(new WritableDatabaseTask<Void>() {
			...
			public Void execute(SQLiteDatabase writableDatabase) {
				@Language("SQL")
				String sql = "UPDATE " + STREAM_TABLE_NAME + " SET " + STREAM_SUBMITTED_FOR_REMOVAL + "= 1 WHERE "
						+ STREAM_MARKED_FOR_REMOVAL + "=1";
